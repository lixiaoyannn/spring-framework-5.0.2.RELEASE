...
//器关闭事件(ContextClosedEvent)类
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//1. 调用容器准备刷新的方法，获取同期的当时时间，同时给容器设置同步标识
			prepareRefresh();
			...
			//3. 为BeanFactory配置同期特性，例如类加载器、事件处理器等
			...
			try {
				...
				//12、初始化同期的声明周期事件处理器，并发布容器的生命周期事件
				finishRefresh();
			}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//这里使用了委派设计模式，父类定义了抽象的refreshBeanFactory()方法，具体实现调用子类容器的refreshBeanFactory()方
		//法
		refreshBeanFactory();
